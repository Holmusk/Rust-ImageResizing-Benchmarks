...
async fn download_img_from_s3 (s3_client:rusoto_s3::S3Client, bucket_name:String, img_name:String) {
        ...
        let s3_object_bytes_mut = match stream.map_ok(|b| bytes::BytesMut::from(&b[..])).try_concat().await {
        ...
        resize_image(bytes_mutref);
           
    }

    fn resize_image(bytes_img : &[u8]){
